    ...
    for doorID, data in pairs(Config.DoorList) do
        if not specificDoor or doorID == specificDoor then
            ...
            elseif not data.doors then
				if not data[6] then data[6] = 'door' end
                if #(playerCoords - data.objCoords) < 30 then
                    if data[6] == "sliding" or data[6] == "garage" then
            ...
            if not data.setText and data.doors then
                for k, v in pairs(data.doors) do
                    ...
                    if k == 2 and data.textCoords and (data[6] == "sliding" or data[6] == "garage") then
...
local function lockpickFinish(success)
	if success then
		NPX.Functions.Notify(Lang:t("success.lockpick_success"), 'success', 2500)
		...
		Wait(1800)
		TriggerServerEvent('rs_doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
	else
		NPX.Functions.Notify(Lang:t("error.lockpick_fail"), 'error', 2500)
		if math.random(1,100) <= 17 then
			if usingAdvanced then
				TriggerServerEvent("rs_doorlock:server:removeLockpick", "advancedlockpick")
				TriggerEvent('inventory:client:ItemBox', NPX.Shared.Items["advancedlockpick"], "remove")
			else
				TriggerServerEvent("rs_doorlock:server:removeLockpick", "lockpick")
				TriggerEvent('inventory:client:ItemBox', NPX.Shared.Items["lockpick"], "remove")
			end
		end
	end
end
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		NPX.Functions.TriggerCallback('rs_doorlock:server:checkItems', function(result)
			p:resolve(result)
...
function SetupDoors()
	local p = promise.new()
	NPX.Functions.TriggerCallback('rs_doorlock:server:setupDoors', function(result)
		p:resolve(result)
	end)
...
RegisterNetEvent('NPX:Client:OnPlayerLoaded', function()
	NPX.Functions.TriggerCallback('rs_doorlock:server:setupDoors', function(result)
...
RegisterNetEvent('NPX:Client:OnPlayerUnload', function()
	isLoggedIn = false
	PlayerData = {}
end)

RegisterNetEvent('NPX:Player:SetPlayerData', function(val)
	PlayerData = val
end)

RegisterNetEvent('rs_doorlock:client:setState', function(serverId, doorID, state, src, enableSounds, enableAnimation)
...
RegisterNetEvent('rs_doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	if not Config.SaveDoorDialog then doorData = {} end
	local dialog = lib.inputDialog('Door Creator', {
		-- title 1
		{type = 'input', label = 'Title', description = 'Title',  min = 1, max = 20},
		-- identifier 2
		{type = 'number', label = 'Model', description = 'Door Identifier', icon = 'hashtag'},
		-- Label 3
		{type = 'input', label = 'Door Label', description = 'Door Label', icon = 'hashtag'},
		-- Type 4
		{type = "select",label = "Door Type",  options = {{ value = 'door', label = 'Door' },{ value = 'double', label = 'Double Door' },{ value = 'sliding', label = 'Sliding' },{ value = 'doublesliding', label = 'Double Sliding' }, { value = 'garage', label = 'Garage' }}},
		-- Distance 5
		{type = 'input', label = 'Distance', description = 'Set door distance'},

		-- Lock type 6
		{type = "select",label = "Door Lock Type / NB! needs fix",  options = {{ value = 'locked', label = 'Locked' },{ value = 'pickable', label = 'Pickable' },{ value = 'cantunlock', label = 'Cant Unlock' },{ value = 'hidelabel', label = 'Hide Label' }}},
		-- Item 7
		{type = 'input', label = 'Item Name',  icon = 'hashtag',  description = 'Item name'} ,
		-- Job 8
		{type = 'input', label = 'Job',  icon = 'hashtag',  description = 'Job name'} ,
		-- Gang 9
		{type = 'input', label = 'Gang',  icon = 'hashtag', description = 'Gang name'},
		-- CID 10
		{type = 'input', label = 'CID',  icon = 'hashtag',  description = 'Citizen ID'},

		-- Locked type thingy 11
		{type = 'checkbox', label = 'Locked?',  icon = 'lock',  description = 'Locked or not'} ,
	...
	if dialog then
		...
		doorData = dialog
		local identifier = doorData[1]..'-'..doorData[2]
		if Config.DoorList[identifier] then
			NPX.Functions.Notify((Lang:t("error.door_identifier_exists")):format(identifier), 'error')
			canContinue = true
			return
		end
		print(json.encode(doorData))
		print(doorData[6])
		if doorData[1] == '' then doorData[1] = false end -- Config
		if doorData[8] == '' then doorData[8] = false end --Job
		if doorData[9] == '' then doorData[9] = false end -- Gang
		if doorData[10] == '' then doorData[10] = false end -- Cid
		if doorData[7] == '' then doorData[7] = false end -- item
		if doorData[3] == '' then doorData[3] = nil end -- Label
		if doorData[6] ~= 'locked' then doorData[6] = nil end
		if doorData[6] ~= 'pickable' then doorData[6] = nil end -- Lock types
		if doorData[6]  ~= 'cantunlock' then doorData[6] = nil end 
		if doorData[6]  ~= 'hidelabel' then doorData[6]  = nil end

		doorData[6] = 'true'
		doorData[5] = tonumber(doorData[5])
		if doorData[4] == 'door' or doorData[4] == 'sliding' or doorData[4] == 'garage' then
			SendNUIMessage({
				type = "setText",
				aim = "block"
			})
			...
			if not model or model == 0 then NPX.Functions.Notify(Lang:t("error.door_not_found"), 'error') canContinue = true return end
			result = DoorSystemFindExistingDoor(coords.x, coords.y, coords.z, model)
			if result then NPX.Functions.Notify(Lang:t("error.door_registered"), 'error') canContinue = true return end
			doorData.doorHash = 'door_'..doorData[2]
			...
			TriggerServerEvent('rs_doorlock:server:saveNewDoor', doorData, false)
			canContinue = true
		else
			...
			if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then NPX.Functions.Notify(Lang:t("error.door_not_found"), 'error') return end
			if entity[1] == entity[2] then NPX.Functions.Notify(Lang:t("error.same_entity"), 'error') canContinue = true return end
			doorData.doorHash = {}
			for i = 1, 2 do
				result = DoorSystemFindExistingDoor(coords[i].x, coords[i].y, coords[i].z, model[i])
				if result then NPX.Functions.Notify(Lang:t("error.door_registered"), 'error') canContinue = true return end
				doorData.doorHash[i] = 'door_'..doorData[2]..'_'..i
			...
			doorData.model = model
			doorData.heading = heading
			TriggerServerEvent('rs_doorlock:server:saveNewDoor', doorData, true)
			canContinue = true
		end
...
RegisterNetEvent('rs_doorlock:client:newDoorAdded', function(data, id, creatorSrc)
	Config.DoorList[id] = data
	TriggerEvent('rs_doorlock:client:setState', creatorSrc, id, data.locked, false, true, true)
end)

RegisterNetEvent('rs_doorlock:client:ToggleDoorDebug', function()
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('rs_doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
...
RegisterCommand('remotetriggerdoor', function()
	...
	TriggerServerEvent('rs_doorlock:server:updateState', nearestDoor.id, not nearestDoor.data.locked, NetworkGetNetworkIdFromEntity(playerPed), false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
